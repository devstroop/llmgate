...
| M10 | Memory substrate & observability | Done. `[memory]` config (fail-closed), embedded nqlite store behind a write-behind actor, per-request records (model, usage, latency, status, request-id) for stream + non-stream paths, TTL purge, `nql-cli` admin queries. |
